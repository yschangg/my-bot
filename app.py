...
# Google Generative AI 라이브러리 (버전에 따라 선택 가능)
try:
    from google import genai
except ImportError:
    st.error("라이브러리 설치가 필요합니다: pip install google-genai")

# ==========================================================
# 1. 시스템 지침 설정 (사용자 제공 지침 원문 100% 유지)
# ==========================================================
...
국문 입력 토큰고정 출력 문자열출력 형식 규칙심사결과EXAMINATION RESULTS대문자, Bold구체적인 거절이유DETAILED REASONS대문자, Bold인용발명ReferenceTitle Case, Bold보정서 제출시 참고사항Notes for AmendmentTitle Case, Bold[첨부]Attachments:Title Case, 콜론 포함, Bold<<안내>>(출력 없음)라인 전체 삭제- 아래 -(출력 없음)라인 전체 삭제[3. 상단 고정 표준 문구 (Introductory Text)]
헤더 바로 아래에 다음 두 문단을 토씨 하나 틀리지 않게 그대로 삽입하십시오.

"According to Article 63 of the Korean Patent Act (KPA), this is to notify the applicant of a preliminary rejection as a result of examination of the present application. The applicant may submit an Argument and/or Amendment by the above response due date."
"The due date can be extended, in principle, for up to four months. The applicant may apply for an extension for one month, or, if necessary, for two or more months at a time. When applying for a time extension in excess of four months based on unavoidable circumstances (see the Guidelines for Time Extensions given below), the applicant is required to submit a justification statement to the Examiner."
...
# 2. Streamlit UI 설정
# ==========================================================
st.set_page_config(page_title="특허 OA 번역 엔진 v2.1", layout="wide")
...
if api_key:
    ...
    # 사용자 입력 처리
    if prompt := st.chat_input("메시지를 입력하세요 (예: '번역 시작', '다음')"):
        ...
        with st.chat_message("assistant"):
            ...
            # 스트리밍 응답 (Batch Output 원칙 및 0.0 온도로 기계적 특성 강화)
    ...
    st.info("왼쪽 사이드바에 Gemini API 키를 입력하여 엔진을 활성화하세요.")
